...
//変更点
extern struct WiFitype wifi;
extern struct FixedIPtype FixedIP;
...
static esp_err_t cmd_handler(httpd_req_t *req) {
  ...
  //Rcat追加分-----------------------------------------------------------------------------------------------------------
    ...
    res = 0;
    FixedIP.IP = val;
  } else if (!strcmp(variable, "mask")) {
    res = 0;
    FixedIP.MASK = val;
  } else if (!strcmp(variable, "gate")) {
    res = 0;
    FixedIP.GATE = val;
  } else if (!strcmp(variable, "fixip")) {
    res = 0;
    FixedIP.Flag = val;
    SaveFixedIPConf();
  }
...
static esp_err_t GoogleDriveOTAUpdate(httpd_req_t *req) {
  ...
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
  if (httpd_query_key_value(buf, "var", variable, sizeof(variable)) != ESP_OK || httpd_query_key_value(buf, "val", value, sizeof(value)) != ESP_OK) {
    url += "15izVVohVk2tF8tfituOVki88LiTHJgiE";
    UpdateFlag = true;
    UpdateURL = url;
    free(buf);
    return httpd_resp_send(req, NULL, 0);
...
static esp_err_t WiFiHtml_handler(httpd_req_t *req) {
  httpd_resp_set_type(req, "text/html");
  httpd_resp_set_hdr(req, "Content-Encoding", "gzip");
  return httpd_resp_send(req, (const char *)wifi_conf, sizeof(wifi_conf));
}
...
void startCameraServer() {
  ...
  httpd_uri_t update_uri = {
    .uri = "/update",
    .method = HTTP_GET,
    .handler = GoogleDriveOTAUpdate,
    .user_ctx = NULL
  };
  httpd_uri_t wifi_html_uri = {
    .uri = "/wifi",
    .method = HTTP_GET,
    .handler = WiFiHtml_handler,
    .user_ctx = NULL
  };
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &update_uri);     //変更点
    httpd_register_uri_handler(camera_httpd, &wifi_html_uri);  //変更点
  }
